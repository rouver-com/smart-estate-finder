  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      type: 'bot',
      content: 'مرحباً! أنا Inspire، مساعدك الذكي للعقارات 🏠 أمتلك معلومات كاملة عن جميع العقارات المتاحة وأستطيع مساعدتك في العثور على العقار المثالي. كيف يمكنني مساعدتك اليوم؟',
      timestamp: new Date()
    }
  ]);
  ...
  const getInspireResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('شقة') || lowerMessage.includes('شقق')) {
      ...
      if (apartments.length > 0) {
        ...
        return `لدي شقق رائعة! على سبيل المثال: "${apt.title}" في ${apt.location} بسعر ${apt.price.toLocaleString()} جنيه. تحتوي على ${apt.bedrooms} غرف نوم و ${apt.bathrooms} حمامات. هل تريد المزيد من التفاصيل؟`;
      }
    ...
    if (lowerMessage.includes('فيلا') || lowerMessage.includes('فلل')) {
      ...
      if (villas.length > 0) {
        ...
        return `عندي فيلا مذهلة لك! "${villa.title}" في ${villa.location} بسعر ${villa.price.toLocaleString()} جنيه. تتميز بـ ${villa.bedrooms} غرف نوم، مساحة ${villa.area} م². هل تريد معاينتها؟`;
      }
      return 'الفلل اختيار ممتاز! لدي فلل فاخرة مع حدائق ومسابح خاصة. ما المنطقة المفضلة لك؟';
    }

    if (lowerMessage.includes('سعر') || lowerMessage.includes('تكلفة') || lowerMessage.includes('كام')) {
      return `أسعارنا تبدأ من 500,000 جنيه للشقق الصغيرة وتصل إلى 20 مليون جنيه للفلل الفاخرة. ما هو النطاق السعري المناسب لك؟ يمكنني ترشيح عقارات تناسب ميزانيتك تماماً!`;
    }

    if (lowerMessage.includes('موقع') || lowerMessage.includes('منطقة') || lowerMessage.includes('فين')) {
      return `لدي عقارات في جميع المناطق الراقية: القاهرة (مدينة نصر، التجمع الخامس، الزمالك)، الإسكندرية (سموحة، سيدي جابر)، الجيزة (المهندسين، الشيخ زايد). أي منطقة تناسبك أكثر؟`;
    }
